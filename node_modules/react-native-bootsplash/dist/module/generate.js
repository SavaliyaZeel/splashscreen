...
export const writeXml = (file, xml, options) => {
...
export const writeHtml = async (file, html, options) => {
...
export const cleanIOSAssets = (dir, prefix) => {
  hfs.readDir(dir).filter(file => file.startsWith(prefix) && file.endsWith(".png")).map(file => path.join(dir, file)).forEach(file => hfs.rm(file));
};
...
export const generate = async ({
  ...
  const logoHeight = await logo.clone().resize(logoWidth).toBuffer().then(buffer => sharp(buffer).metadata()).then(({
    height = 0
  }) => Math.round(height));
  ...
  if (iosProjectPath != null) {
    ...
    addFileToXcodeProject(path.join(path.basename(iosProjectPath), "BootSplash.storyboard"));
